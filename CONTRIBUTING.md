# Contributing to ep_rss

Thanks for helping improve `ep_rss` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_rss` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_rss from your local clone
pnpm run plugins i --path ../ep_rss

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_rss/` are picked up after a server restart.
...
* Etherpad version, ep_rss version, browser + OS
* Steps to reproduce
